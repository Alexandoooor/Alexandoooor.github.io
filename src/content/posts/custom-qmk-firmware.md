...
After flashing the updated firmware, the timing deciding whether an action is a `tap` or a `hold` had changed.
...
There is the [tapping-term](https://docs.qmk.fm/tap_hold#tapping-term)-option, which controls how long you need to hold the key before it is interpreted as a `hold`. There is also an option for different [Tap-Or-Hold Decision Modes](https://docs.qmk.fm/tap_hold#tap-or-hold-decision-modes).
...
I could set it up for use with the QMK-cli by running the following command:
```bash
...
As mentioned before, tapping term controls how long you need to hold the key pressed for it to be interpreted as `hold`.
